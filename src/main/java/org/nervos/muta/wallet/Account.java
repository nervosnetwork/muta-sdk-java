...
public class Account {
  ...
  private Account(byte[] privateKey) {
    ...
    this.point =
        CryptoUtil.publicPoint(new BigInteger(Arrays.concatenate(new byte[] {0x00}, privateKey)));
  ...
  public byte[] sign(byte[] msgHash) {
    byte[] ret =
        CryptoUtil.sign(new BigInteger(Arrays.concatenate(new byte[] {0x00}, privateKey)), msgHash);
    return ret;
  }
}
