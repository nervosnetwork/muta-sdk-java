...
public class Client {
  ...
  protected <T> T parseGraphQlResponse(@NonNull Response response, String operation, Class<T> clazz)
      throws IOException {
  ...
  public Block getBlock(Uint64 height) throws IOException {
    MutaRequest mutaRequest =
        new MutaRequest(
            GetBlockRequest.operation, new GetBlockRequest(height), GetBlockRequest.query);
  ...
  public SignedTransaction getTransaction(Hash txHash) throws IOException {
    MutaRequest mutaRequest =
        new MutaRequest(
  ...
  public Receipt getReceipt(Hash txHash) throws IOException {
    MutaRequest mutaRequest =
        new MutaRequest(
            GetReceiptRequest.operation, new GetReceiptRequest(txHash), GetReceiptRequest.query);
  ...
  public ServiceResponse queryService(
    ...
    ServiceResponse ret =
        this.parseGraphQlResponse(response, QueryServiceRequest.operation, ServiceResponse.class);

    return ret;
  }

  public Hash sendTransaction(
      InputRawTransaction inputRaw, InputTransactionEncryption inputEncryption) throws IOException {
    MutaRequest mutaRequest =
        new MutaRequest(
    ...
    Hash ret = this.parseGraphQlResponse(response, SendTransactionRequest.operation, Hash.class);
    return ret;
  }
}
