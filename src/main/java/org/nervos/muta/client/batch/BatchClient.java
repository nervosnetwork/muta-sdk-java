...
public class BatchClient extends Client {
    ...
    public List<BatchQueryResponse> query(List<BatchQuery> batchQueries) throws IOException {
        ...
        for (int index = 0; index < batchQueries.size(); index++) {
            ...
            String paramNameWithType = "$"+item.getBatchParamPrefix()+index+":"+item.getBatchParamType();
    ...
    public List<BatchQueryResponse> parse_batch(@NonNull Response response, List<BatchQuery> batchQueries) throws IOException {
    ...
    public List<BatchQueryResponse> do_parse(JsonNode dataRoot, List<BatchQuery> batchQueries) throws IOException {
        ...
        for (int index = 0; index < batchQueries.size(); index++) {
            ...
            if(item instanceof GetBlockRequest){
                ret.add(this.getObjectMapper().treeToValue(dataRoot.get(item.getBatchAliasPrefix()+index), Block.class));
            }else if (item instanceof GetReceiptRequest){
                ret.add(this.getObjectMapper().treeToValue(dataRoot.get(item.getBatchAliasPrefix()+index), Receipt.class));
            }else if (item instanceof GetTransactionRequest){
                ret.add(this.getObjectMapper().treeToValue(dataRoot.get(item.getBatchAliasPrefix()+index), Transaction.class));
            }else {
